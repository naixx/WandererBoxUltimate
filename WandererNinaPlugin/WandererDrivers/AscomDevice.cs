...
namespace Naixx.NINA.Wanderer.WandererDrivers {
    ...
    public abstract class AscomDevice<DeviceT> : BaseINPC, IDevice
                ...
                try {
                    ...
                    if (Connected) {
                        ...
                        try {
                            ...
                            lastDeviceState = new List<StateValue>();
        ...
        protected PropT GetProperty<PropT>(string propertyName, PropT defaultValue, TimeSpan? cacheInterval = null, bool rethrow = false,
            bool useLastKnownValueOnError = true, PropT errorValue = default) {
            ...
            for (int i = 0; i < retries; i++) {
                try {
                    ...
                    if (memory.ConsecutiveErrors == memory.ConsecutiveErrorThreshold) {
                        Logger.Warning(
                            $"GET of {type.Name}.{propertyName} encountered {memory.ConsecutiveErrorThreshold} consecutive errors. Further logs for this property access are logged on TRACE level until the property access is successful again");
                    }

                    if (ex is PropertyNotImplementedException || ex.InnerException is PropertyNotImplementedException
                                                              || ex is ASCOM.NotImplementedException ||
                                                              ex.InnerException is ASCOM.NotImplementedException
                    ...
                    if (memory.ConsecutiveErrors > memory.ConsecutiveErrorThreshold) {
                        Logger.Trace(
                            $"An unexpected exception occurred during GET of {type.Name}.{propertyName} - Consecutive Errors: {memory.ConsecutiveErrors} - Error: {logEx.Message} {logEx.StackTrace}");
                    } else {
            ...
            if (memory.ConsecutiveErrors > memory.ConsecutiveErrorThreshold) {
                Logger.Trace(
                    $"GET {type.Name}.{propertyName} failed - Returning {(useLastKnownValueOnError ? "last known" : "error")} value {val}");
            } else {
                Logger.Info(
                    $"GET {type.Name}.{propertyName} failed - Returning {(useLastKnownValueOnError ? "last known" : "error")} value {val}");
            }
        ...
        /// <returns></returns>
        protected bool SetProperty<PropT>(string propertyName, PropT value, TimeSpan? cacheInterval = null,
            [CallerMemberName] string originalPropertyName = null) {
            if (device != null) {
